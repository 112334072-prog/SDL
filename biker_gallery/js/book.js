...
function renderSlots() {
  ...
  /* ── Hide past slots when today is selected ──
     A slot is "past" if its hour <= current hour on today's date.
     We hide (not just disable) past slots so the grid stays clean. */
  ...
  ALL_SLOTS.forEach(slot => {
    /* Hide slots that have already passed (only on today) */
    if (isToday && slot.hour <= currentHour) {
      return; /* skip — don't render this button at all */
    }
  ...
  /* Show message if no slots left today */
  if (!anyVisible) {
    const msg = document.createElement('div');
    msg.className = 'slots-loading';
    msg.style.cssText = 'color:#ff5370;font-size:13px;padding:16px 0;text-align:center;';
    msg.textContent = 'No more slots available for today. Please select a future date.';
