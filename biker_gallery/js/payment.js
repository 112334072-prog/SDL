...
function setConfirmState(state, opts = {}) {
  ...
  const _dn = document.getElementById('confirmDocNote');
  if (_dn) _dn.style.display = 'none';

  if (state === 'confirm') {
    icon.textContent  = '!';
    title.textContent = opts.title || 'Confirm Payment';
    text.innerHTML    = opts.text ? opts.text.replace(/\n/g, '<br>') : (confirmDefaultText ? confirmDefaultText.replace(/\n/g, '<br>') : 'Click OK to proceed.');
    ...
    text.innerHTML    = opts.text ? opts.text.replace(/\n/g, '<br>') : 'Your booking has been confirmed.';
    okBtn.style.display = 'none'; cancelBtn.style.display = 'none';
    actions2.style.display = '';
    const docNoteEl = document.getElementById('confirmDocNote');
    if (docNoteEl) docNoteEl.style.display = '';
  } else if (state === 'fail') {
...
async function processPayment(method, opts = {}) {
  ...
  try {
    ...
    const depositLine = `Deposit Paid (Refundable): ${fmt(bookingData.deposit || 2500)}`;
