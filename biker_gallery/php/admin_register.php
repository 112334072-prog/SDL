...
if (empty($uname1) || empty($full_name) || empty($email) ||
    ...
    jsGo('../html/admin_register.html?msg=All+fields+are+required');
}

/* Username length */
if (strlen($uname1) < 4) {
    jsGo('../html/admin_register.html?msg=Username+must+be+at+least+4+characters');
}

/* Passwords must match */
if ($upswd1 !== $upswd2) {
    jsGo('../html/admin_register.html?msg=Passwords+do+not+match');
}
...
if ($stmt->num_rows > 0) {
    ...
    jsGo('../html/admin_register.html?msg=Email+already+registered');
}
$stmt->close();
...
if ($stmt->num_rows > 0) {
    $stmt->close(); $conn->close();
    jsGo('../html/admin_register.html?msg=Username+already+taken');
}
$stmt->close();
...
if ($stmt->execute()) {
    ...
    $stmt->close(); $conn->close();
    jsGo('../html/admin_register.html?msg=Registration+failed.+Please+try+again');
}
