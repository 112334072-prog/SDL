...
/* ============================================================
   ...
   login_authentication == '1'     → check users table       → ../html/home.html
...
if (!$uname || !$upswd) {
    jsGo('../html/login.html?msg=Username+and+password+are+required');
}
...
if ($auth_type === 'admin') {
    ...
    if ($found === 1) {
        ...
        if (password_verify($upswd, $password_db)) {
            ...
            $conn->close();
            jsGo('../html/login.html?msg=Invalid+admin+username+or+password');
        }
    } else {
        $stmt->close();
        $conn->close();
    ...
    if ($found === 1) {
        ...
        if (password_verify($upswd, $password_db)) {
            ...
            $conn->close();
            jsGo('../html/login.html?msg=Invalid+username+or+password');
        }
    } else {
        $stmt->close();
        $conn->close();
