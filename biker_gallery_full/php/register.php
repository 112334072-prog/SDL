...
if (empty($uname1) || empty($full_name) || empty($dob) ||
    ...
    jsGo('../register.html?msg=All+fields+are+required');
}
if (strlen($uname1) < 4) {
    jsGo('../register.html?msg=Username+must+be+at+least+4+characters');
}
if ($upswd1 !== $upswd2) {
    jsGo('../register.html?msg=Passwords+do+not+match');
}
...
if ($stmt->num_rows > 0) {
    ...
    jsGo('../register.html?msg=Email+already+registered');
}
$stmt->close();
...
if ($stmt->num_rows > 0) {
    $stmt->close(); $conn->close();
    jsGo('../register.html?msg=Username+already+taken');
}
$stmt->close();
...
if ($stmt->execute()) {
    ...
    $stmt->close(); $conn->close();
    jsGo('../register.html?msg=Registration+failed.+Please+try+again');
}
